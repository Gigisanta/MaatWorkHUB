...
export function SyncStatus({ appId }: { appId: string }) {
  ...
  if (isLoading)
    return <div className="animate-pulse h-4 w-24 bg-white/5 rounded" />;
  ...
  if (status?.status === "out_of_sync") {
    return (
      <div className="flex items-center gap-3">
        <Badge
          variant="outline"
          className="text-[10px] text-yellow-500 border-yellow-500/20 bg-yellow-500/5"
        >
          <AlertTriangle className="w-3 h-3 mr-1" /> {status.behindBy} commits
          atrás
        </Badge>
        <Button
          size="sm"
