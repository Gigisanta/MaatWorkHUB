...
import { ArrowLeft, Users, Building, Activity, ShieldCheck, Receipt, BarChart3, Database } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="infrastructure" className="space-y-6">
                ...
                <Card className="border-white/5 bg-black/40 backdrop-blur-3xl">
                    ...
                    <CardContent className="space-y-6">
                        <div className="space-y-1">
                            <p className="text-[10px] text-white/20 uppercase font-black tracking-widest">Internal ID</p>
                            <p className="text-xs text-white/60 font-mono break-all bg-white/5 p-2 rounded border border-white/5">{app.id}</p>
                        </div>
                        <div className="space-y-1">
                            <p className="text-[10px] text-white/20 uppercase font-black tracking-widest">Custom Domain</p>
                            <p className="text-sm text-white/80 font-semibold">{app.domain || "Ninguno configurado"}</p>
                        </div>
                        <div className="space-y-1">
                            <p className="text-[10px] text-white/20 uppercase font-black tracking-widest">Estado Aprovisionamiento</p>
                            <p className="text-sm text-primary font-bold uppercase italic">{app.provisioningStatus || 'desconocido'}</p>
                        </div>
                        <div className="pt-4 border-t border-white/5 flex flex-col gap-2">
                            <Button variant="outline" size="sm" className="w-full border-white/5 hover:bg-white/5 text-[10px] uppercase font-black">Refrescar Salud</Button>
                            <Button variant="destructive" size="sm" className="w-full text-[10px] uppercase font-black">Suspender Aplicación</Button>
                        </div>
                    </CardContent>
                </Card>
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-2">
                ...
                <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                        <Building className="w-5 h-5 text-primary" /> Admins & Empleados
                    </CardTitle>
                    <CardDescription>Personas con acceso al panel administrativo de esta app.</CardDescription>
                </CardHeader>
                <CardContent>
                    <Table>
                    <TableHeader>
                        <TableRow className="border-white/5 hover:bg-transparent">
                        <TableHead className="text-[10px] uppercase font-black text-white/40">Email</TableHead>
                        <TableHead className="text-[10px] uppercase font-black text-white/40">Rol</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {appUsers.length === 0 ? (
                        ...
                        appUsers.map((user) => (
                            ...
                            <TableCell className="font-medium text-white/80">{user.email}</TableCell>
                            <TableCell className="capitalize text-white/40 font-mono text-xs">{user.role}</TableCell>
                            </TableRow>
                        ))
                        )}
                    </TableBody>
                ...
                <Card className="border-white/5 bg-black/40 backdrop-blur-3xl">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                            <Users className="w-5 h-5 text-blue-400" /> Clientes Registrados
                        </CardTitle>
                        <CardDescription>Usuarios finales gestionados por este centro.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="flex flex-col items-center justify-center py-12 border-2 border-dashed border-white/5 rounded-2xl opacity-40">
                             ...
                             <p className="text-[10px] uppercase tracking-widest mt-1">Clientes Totales</p>
                             <Button variant="link" size="sm" className="mt-4 text-primary" asChild>
        ...
        <TabsContent value="billing" className="outline-none">
            <Suspense fallback={<div className="h-64 flex items-center justify-center border border-dashed border-white/10 rounded-2xl"><Skeleton className="h-32 w-full max-w-md" /></div>}>
