...
export default async function AppManagementPage({ 
  ...
  if (!appRecord) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        ...
        <p className="text-white/40 text-sm italic">Slug buscado: &quot;{appSlug}&quot;</p>
  ...
  const activityItems: ActivityItem[] = logs.map(log => ({
    ...
    icon: log.action.includes('APP') ? <Building2 className="h-4 w-4" /> : 
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-4xl font-black tracking-tight text-white mb-2">{appRecord.name}</h1>
          <div className="flex items-center gap-4 text-xs text-white/40">
            <span className="flex items-center gap-1.5 text-green-400/80">
              <ShieldCheck className="h-3.5 w-3.5" />
              Suscripción Pro
            </span>
            <span className="uppercase tracking-widest border-l border-white/10 pl-4">{appRecord.template}</span>
          </div>
        </div>
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-green-500/10 border border-green-500/20">
                <div className="text-xs font-bold text-green-400 uppercase mb-1">Salud del App</div>
