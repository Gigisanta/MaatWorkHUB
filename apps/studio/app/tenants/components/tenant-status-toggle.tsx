...
export function TenantStatusToggle({ tenantId, currentStatus }: TenantStatusToggleProps) {
  const { execute, status } = useAction(toggleTenantStatusAction, {
    onSuccess: (data) => {
      toast.success(
  ...
  return (
    <Button
      ...
      size="sm"
      disabled={isLoading}
      onClick={() => execute({ tenantId, currentStatus })}
      className="gap-2"
    >
      {isLoading ? (
