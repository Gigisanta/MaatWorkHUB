...
export default async function AgendaPage({ params }: { params: Promise<{ app: string }> }) {
  const resolvedParams = await params;
  
  const appRecord = await db.query.apps.findFirst({
    where: eq(apps.slug, resolvedParams.app),
  });

  if (!appRecord) return <div className="p-8 text-center text-red-500 font-bold">App not found</div>;
