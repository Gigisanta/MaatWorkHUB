...
function main() {
  ...
  for (const dirName of DIRS_TO_CLEAN) {
    try {
      ...
      execSync(`find . -name "${dirName}" -type d -prune -exec rm -rf '{}' +`, { stdio: 'inherit' });
    } catch (e) {
      console.warn(`⚠️  Failed to clean some ${dirName} instances. Continuing...`);
    }
  }
  ...
  console.log('✨ Cleanup complete! Run "pnpm install" to restore dependencies.');
}
