...
export async function checkSystemHealth(): Promise<HealthStatus[]> {
  ...
  try {
    ...
    if (!VERCEL_TOKEN) {
      results.push({
        service: "vercel",
        status: "failed",
        message: "Token de Vercel no configurado",
      });
    } else {
      ...
      results.push({
        service: "vercel",
        status: "operational",
        message: "Servicio de Vercel activo",
      });
    }
  } catch {
  ...
  try {
    ...
    results.push({
      ...
      message: NEON_API_KEY
        ? "API de Neon activa"
        : "API Key de Neon no configurada",
    });
  } catch {
...
export async function checkProjectHealth(
  ...
  return {
    ...
    overall:
      githubStatus === "ok" &&
      (vercelStatus === "READY" || vercelStatus === "unknown")
        ? "healthy"
        : "degraded",
  };
}
